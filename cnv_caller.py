...
def load_arguments():
    ...
    misc.add_argument('-f', '--fetal', type=float, default=0.1, help="Approximate fetal fraction. Default:0.1")
    misc.add_argument('-s', '--height-signif', type=positive_float, default=0.75,
                      help="How sensitive is the caller (when is a variant considered significant - crimson/red/yellow). Trade-off between sensitivity and specificity. Should be in (0.4 - 1.0)")
    misc.add_argument('-x', '--x-count', type=int, default=None, help="Number of X chromosomes of the fetus. 1 for a healthy boy, 2 for a healthy girl. Used only for gonosomal chromosomes.")
    misc.add_argument('-l', '--min-length', type=int, default=3000000,
                      help="Minimal effective length of fetal detection (in bases) to report. Supply -1 for infinity, 0 for no restriction. Default=3,000,000.")
    ...
    # adjust possible wrong arguments:
    if args.fetal > 1.0:
        print('WARNING: fetal fraction should be between ~5%% and 100%% (ff= %6.2f%%), clipping...' % (float(args.fetal) * 100.0))
        args.fetal = np.clip(args.fetal, 0.0, 1.0)
    if args.fetal < 0.0:
        print('WARNING: fetal fraction should be between ~5%% and 100%% (ff= %6.2f%%), assigning default value 0.1' % (float(args.fetal) * 100.0))
        args.fetal = 0.1
...
def get_significance(mean_bins, cbs, ff, height_signif, signif_prefix, ind_good=None, window=20000, x_count=None, min_length=None, genomic=False):
    """
    ...
    :param cbs: pd.DataFrame - CBS smoothing of bins
    :param ff: float - approximate fetal fraction
    :param height_signif: float - relative significance of CNV levels
    :param signif_prefix: str - filename to write the significance to
    ...
    :param min_length: int - minimal length for fetal detection
    :param genomic: bool - genomic sample?
    :return: pd.DataFrame, (float, float, float, float, float, float) - CBS smoothing of bins with significance info, expected fetal and maternal levels for autosomal, X and Y chromosome
    """
    ...
    # significance levels
    yellow_sign_real = 0.5 * avg_bin * ff / 2.0
    red_sign_real = avg_bin * ff / 2.0
    crimson_sign_real = avg_bin * (1 - ff) / 2.0
    if genomic:
    ...
    if x_count is not None and len(mean_bins) == len(ind_good):
        ...
        added_value_fetal = ff / 2.0
        added_value_mater = (1 - ff) / 2.0
        if x_count == 1:  # here the added value is dependent on fetal fraction in non-trivial way since the amount of X chrom. is dependent on fetal fraction
            added_value_fetal = ff / (2 - ff)
            added_value_mater = (1 - ff) / (2 - ff)
        yellow_sign_real_x = 0.5 * avg_bin_x * added_value_fetal
        red_sign_real_x = avg_bin_x * added_value_fetal
        crimson_sign_real_x = avg_bin_x * added_value_mater
        if crimson_sign_real_x < red_sign_real_x:
            crimson_sign_real_x = np.nan
        if genomic:
    ...
    if x_count is not None and len(mean_bins) == len(ind_good) and x_count == 1:
        ...
        yellow_sign_real_y = 0.5 * avg_bin_y
        red_sign_real_y = 1.0 * avg_bin_y
        crimson_sign_real_y = np.inf
        if genomic:
    ...
    yellow_sign = max(min(0.1, yellow_sign_real), height_signif * yellow_sign_real)
    orange_sign = max(min(0.15, red_sign_real), height_signif * red_sign_real)
    red_sign = max(min(0.2, red_sign_real), height_signif * red_sign_real)
    crimson_sign = max(1.0, height_signif * crimson_sign_real)
    if crimson_sign <= red_sign:
        crimson_sign = np.nan
    if crimson_sign_real <= red_sign_real:
        crimson_sign_real = np.nan
    if genomic:
        yellow_sign = max(0.1, 0.5 * yellow_sign_real)
        orange_sign = max(0.15, 0.5 * red_sign_real)
        red_sign = max(0.2, 0.5 * red_sign_real)
        crimson_sign = max(1.0, 0.75 * crimson_sign_real)  # fetal fraction is not there in genomic
    ...
    if yellow_sign_real_x is not None:
        yellow_sign_x = max(min(0.1, yellow_sign_real_x), height_signif * yellow_sign_real_x)
        orange_sign_x = max(min(0.15, red_sign_real_x), height_signif * red_sign_real_x)
        red_sign_x = max(min(0.2, red_sign_real_x), height_signif * red_sign_real_x)
        crimson_sign_x = max(1.0, height_signif * crimson_sign_real_x)
        if crimson_sign_x <= red_sign_x:
            crimson_sign_x = np.nan
        if crimson_sign_real_x <= red_sign_real_x:
            crimson_sign_real_x = np.nan
        if genomic:
            ...
            crimson_sign_x = max(1.0, 0.75 * crimson_sign_real_x)  # fetal fraction is not there in genomic
    ...
    if genomic:
        print("Significance levels (genomic): %5.3f (%5.3f - yellow) %5.3f (%5.3f - orange) %5.3f (%5.3f - red) %5.3f (%5.3f - magenta) (%5.3f avg_bin)" % (
            yellow_sign, yellow_sign_real, orange_sign, red_sign_real, red_sign, red_sign_real, crimson_sign, crimson_sign_real, avg_bin))
        if yellow_sign_x is not None:
            print("Significance levels (genomic) X: %5.3f yellow %5.3f orange %5.3f red %5.3f magenta (%5.3f avg_bin)" % (
                yellow_sign_x, orange_sign_x, red_sign_x, crimson_sign_x, avg_bin_x))
        if yellow_sign_y is not None:
            print("Significance levels (genomic) Y: %5.3f yellow %5.3f orange %5.3f red %5.3f magenta (%5.3f avg_bin)" % (
                yellow_sign_y, orange_sign_y, red_sign_y, crimson_sign_y, avg_bin_y))
    else:
        print("Significance levels: %5.3f (%5.3f - yellow) %5.3f (%5.3f - orange) %5.3f (%5.3f - red) %5.3f (%5.3f - maternal) (%5.3f avg_bin, %4.2f height_signif, %5.2f%% ff)" % (
            yellow_sign, height_signif * avg_bin * (ff / 2) / 2, orange_sign, height_signif * avg_bin * (ff / 2), red_sign, height_signif * avg_bin * (ff / 2), crimson_sign,
            height_signif * (avg_bin * (1 - ff) * 3 / 2 + avg_bin * ff - avg_bin), avg_bin, height_signif, ff * 100.0))
        if yellow_sign_x is not None:
            print("Significance levels X: %5.3f yellow %5.3f orange %5.3f red %5.3f maternal (%5.3f avg_bin)" % (
                yellow_sign_x, orange_sign_x, red_sign_x, crimson_sign_x, avg_bin_x))
        if yellow_sign_y is not None:
            print("Significance levels Y: %5.3f yellow %5.3f orange %5.3f red %5.3f maternal (%5.3f avg_bin)" % (
                yellow_sign_y, orange_sign_y, red_sign_y, crimson_sign_y, avg_bin_y))

    def assign_significance(chromosome, level, length, min_length=None, genomic=False):
        """
        ...
        if used_signs is None:
            if abs(level) > 10:  # return orange if level is too much
                return 2
            return -1
        ...
        min_lengths = np.array([0, 20, 30, min_length / 20000, 10])
        if genomic:
    ...
    significant = assign_significance_vec(cbs["chromosome"], cbs["level"], cbs["effective_length"], min_length, genomic)
    ...
    def expected_ff(level, avg_bin, avg_bin_x, avg_bin_y, chromosome, x_count, ff):
        """
        ...
        if chromosome == 22:
            if avg_bin_x is None or x_count == 0:
                ...
                elif x_count == 1:  # here the added value is dependent on fetal fraction in non-trivial way since the amount of X chrom. is dependent on fetal fraction
                    return np.abs(level / avg_bin_x * (2 - ff))
                else:
                    return 0.0
        elif chromosome == 23:
    ...
    expected_ffs = [expected_ff(cbs['level'][i], avg_bin, avg_bin_x, avg_bin_y, cbs['chromosome'][i], x_count, ff) for i in cbs.index]
    cbs['expected_ff'] = pd.Series(expected_ffs, index=cbs.index)

    # write the significance levels to a file
    if signif_prefix is not None:
        signif_filename = '%s_signif_levels.txt' % signif_prefix

        with open(signif_filename, 'w') as f:
            print('green %.3f %.3f' % (0.0, yellow_sign), file=f)
            print('yellow %.3f %.3f' % (yellow_sign, orange_sign), file=f)
            print('orange %.3f %.3f' % (orange_sign, crimson_sign), file=f)
            print('red %.3f %.3f' % (red_sign, crimson_sign), file=f)
            print('magenta %.3f %.3f' % (crimson_sign, 9.9), file=f)

            print('red_real %.3f' % red_sign_real, file=f)
            print('magenta_real %.3f' % crimson_sign_real, file=f)
...
if __name__ == "__main__":
    ...
    cbs, expected_levels = get_significance(mean_data, cbs_r, args.fetal, args.height_signif, None, ind_filtered, x_count=args.x_count,
